...
1. **水印中文**：已支持——水印改用 canvas 渲染整页 PNG（浏览器字体天然含中文），无需捆绑 CJK 字体文件；页码为数字，Helvetica 即可。
...
- **水印支持中文（第五轮）**：水印由"标准字体 Helvetica 绘制"改为 **canvas 渲染整页水印 PNG**（浏览器字体含中文），`embedPng` 后每页 `drawImage` 一次。彻底解决"中文水印不显示/乱码"，且无需捆绑 CJK 字体文件。
- **栅格化引擎重写——根治卡死 + 分页不连贯（第五轮，核心）**：
  - 旧版把整元素抓成一张超长图再切片，**长文档/多工作表会生成数万像素高的 canvas，内存瞬间爆掉→卡死/崩溃**（Excel 特别明显）。
  - 新版 `html2pdf.ts` 改为**按 A4 内容高度逐片渲染**：每片仅约一页高，canvas 像素高度恒定有界（实测约 2300px/片，峰值内存 ≈14MB，与文档长度无关）；然后 `pdf-lib` 合并各工作表为单文件（Excel 每表独立宿主→各自内存有界、从新页开始）。
  - 分页连贯性：切片边界主动避让**文字块(`p/li/h1-6/blockquote/pre`)与图片/表格**，在 ±48px 窗口内找最靠近原切页线、且不在任何块内的位置，使段落/标题/图片尽量完整落在同一页，解决"分页不连贯、文字被腰斩"。

**仍待提升（M5 候选 / 更高成本）：**
- 栅格化转换（Office→PDF）文本不可选：需引入 LibreOffice headless 或服务端 API（超出纯前端范畴）。
- 压缩对纯矢量 PDF 可能变大（栅格化本质）。
- PDF→文本对扫描件为空：需 OCR（tesseract.js，M5）。
