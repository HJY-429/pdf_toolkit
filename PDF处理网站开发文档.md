...
- `src/ui/ui.ts`：通用「首页分类卡片网格 → 选择工具 → 上传 → 参数 → 处理 → 下载」流程；首页按 `tool-meta.ts` 的分类（合并与拆分/页面编辑/格式转换/安全加密/优化压缩）分组展示 16 个工具图标卡片；自动按 `fields` 渲染表单；多文件显示排序/删除；进度条；结果逐个下载或「全部下载」（>6 个文件自动打包 ZIP）；支持深浅主题切换（localStorage 记忆）。
...
- **首页 = 分类卡片网格**：按 5 个分类分组展示 16 个工具卡片（图标 + 标题 + 描述），`auto-fill minmax` 响应式网格；点击卡片进入对应工作台。
...
- **新增 PDF→Excel 工具**（`pdf-to-excel`，纯前端）：pdf.js 逐页提取**带坐标**的文本项，按 y 聚行为"行"、按 x 聚类为"列"，重建成 2D 网格写入 XLSX（SheetJS 已有依赖），并据每列最长文本估算列宽。能保留文本与大致版面/阅读顺序；**局限**：复杂合并单元格、无文本层的扫描件无法完美还原（PDF 天生无表格结构）。
